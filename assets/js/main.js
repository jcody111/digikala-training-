...
const dropSortItem = document.querySelectorAll('.dropped-sort-item')
...
for (let i = 0; i < sortItem.length; i++) {
    sortItem[i].onclick = () => {
        for (let j = 0; j < dropSortItem.length; j++) {
            dropSortItem[j].classList.remove('active')
        }
        dropSortItem[i].classList.add('active')
    }
}
